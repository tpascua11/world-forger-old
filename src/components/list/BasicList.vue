<template>
  <div class="container dt-border-x2">
    <section class="">
      ...
      <div id="item-list" class="cool-scroll" v-bind:style="{height: box_height}">
      ...
      <div class="border-top-x2">
        <button v-on:click="addNewRow(); scrollToEnd();" class="pure-button full-width">
          Create
        </button>
      </div>
    </section>
...
export default {
  ...
  props: ['value', 'mapName', 'set_height'],
  mounted(){
  },
  methods:{
    ...
    addNewRow: function(){
      let index = Object.keys(this.list).length+1;
      let newEntity = {id: index};
      this.$emit('input', newEntity);
      this.$root.world[this.mapName][index] = newEntity;
  ...
  computed: {
    ...
    list: function(){
      return this.$root.world[this.mapName];
    },
    ritemMap: function(){
